...
def ready(order):
    ...
    wait_queue.append((1, 0, domain, pid))
...
def request(order):
    ...
    # [1-2] 아니라면,
    # 대기큐에 입력 (우선순위, 초, 도메인, 문제번호)
    wait_queue.append((p, t, domain, pid))
...
def mark(order):
    ...
    for task_idx, task in enumerate(wait_queue):
...
if __name__ == "__main__":
    ...
    for idx in range(Q):
        ...
        wait_queue.sort(key = cmp_to_key(priority))